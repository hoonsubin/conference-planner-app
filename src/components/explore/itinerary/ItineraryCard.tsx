...
import { airplane, location, link, calendarOutline, timeOutline } from 'ionicons/icons';
...
const ItineraryCard: React.FC<ItineraryCardProps> = ({ itinerary, allItineraryOptions, attendee, selectMode, isSelected, setSelectedFlightPlan }) => {
  ...
  return (
    ...
    >
      ...
      >
        <IonCardTitle>{attendee ? attendee?.name : itinerary.airline}</IonCardTitle>
        {
            selectMode ?
      ...
      <IonCardContent style={{ display: 'flex', flexDirection: 'column', paddingTop: '32px', columnGap: '16px' }}>
        ...
        <div style={{'display': 'flex', 'flexDirection': 'row', 'rowGap': '10px', 'alignItems': 'center'}}>
            <IonIcon icon={calendarOutline} style={
                {'marginRight': '10px', 'marginLeft': '0px',  fontSize: '24px'}
            }></IonIcon>
            <div>
                <span>
                    {itinerary.departTime ? dayjs(itinerary.departTime.toLocaleString()).format('DD.MM.YYYY') : "no date available"}
                </span>
            </div>
        </div>
        <div style={{'display': 'flex', 'flexDirection': 'row', 'rowGap': '10px', 'alignItems': 'center', 'marginLeft': '20px'}}>
            <IonIcon icon={timeOutline} style={{'marginRight': '10px', 'marginLeft': '0px', fontSize: '24px'}}></IonIcon>
            <div>
                <span>
                    {itinerary.departTime ? dayjs(itinerary.departTime.toLocaleString()).format('h:mm') : "no date available"}
                </span>
            </div>
        </div>
        </IonText>
        ...
        >
          ...
          ></div>
          <IonIcon icon={airplane} className="ion-padding-end" style={{color: "#3b82f6"}}  />
          <div>
            <IonText>
        ...
        <div style={{'display': 'flex', 'flexDirection': 'row', 'rowGap': '10px', 'alignItems': 'center'}}>
            ...
            <div>
                <span>
                    {itinerary.arrivalTime ? dayjs(itinerary.arrivalTime.toLocaleString()).format('DD.MM.YYYY') : "no date available"}
                </span>
            </div>
        </div>
        <div style={{'display': 'flex', 'flexDirection': 'row', 'rowGap': '10px', 'alignItems': 'center', 'marginLeft': '20px'}}>
            ...
            <div>
                <span>
                    {itinerary.arrivalTime ? dayjs(itinerary.arrivalTime.toLocaleString()).format('h:mm') : "no date available"}
                </span>
            </div>
        </div>
        </IonText>
        ...
        {/* Booking Link */}
        <div>
          <IonIcon icon={link} className="ion-padding-end" style={{fontSize: "24px"}} />
          <IonText>Booking Link:</IonText>
        </div>
        <br />
        <IonText>
          <a href={itinerary.bookingLink} target="_blank" rel="noopener noreferrer">
