import {
  ...
  const EventDetail: React.FC<EventDetailProps> = () => {
    ...
    const data = loc.state as { event: ConferenceEvent; isSaved: boolean } | undefined;
    const {
      saveConferenceEvent,
    ...
    function getAttendeeIds() {
      ...
      return event
        ? allAttendeeItinerary.filter((itinerary) => itinerary.eventId === event.id)
        : [];
    }
    ...
    function checkIfJourneyExists() {
      return event ? allAttendeeItinerary.some((itinerary) => itinerary.eventId === event.id) : false;
    }
          ...
          <IonContent className="ion-padding detail-content">
            <IonCard className="event-detail-card">
              <IonCardContent className="flex-space-card ion-padding">
                ...
                <IonIcon
                  ...
                  size="large"
                  style={{ marginRight: "8px", color: isSaved ? "red" : "var(--ion-color-primary-contrast)" }}
            ...
            <IonCard className="event-detail-card">
              <IonCardContent className="ion-padding" style={{ display: "flex", flexDirection: "row", justifyContent: "start" }}>
                <div style={{ display: "flex", flexDirection: "row", rowGap: "10px", alignItems: "center" }}>
                  <IonIcon icon={calendarOutline} size="large" style={{ marginRight: "10px", marginLeft: "10px" }} />
                  <div>
                    <span style={{ fontWeight: "bold" }}>Date</span>
                    <br />
                    <span>{event.eventStartDate ? dayjs(event.eventStartDate.toLocaleString()).format("MMMM D, YYYY") : "no date available"}</span>
                  </div>
                </div>
                <div style={{ display: "flex", flexDirection: "row", rowGap: "10px", alignItems: "center", marginLeft: "20px" }}>
                  <IonIcon icon={timeOutline} size="large" style={{ marginRight: "10px", marginLeft: "10px" }} />
                  <div>
                    <span style={{ fontWeight: "bold" }}>Time</span>
                    <br />
                    <span>{event.eventStartDate ? dayjs(event.eventStartDate.toLocaleString()).format("h:mm A") : "no time available"}</span>
                  </div>
                </div>
              </IonCardContent>
            </IonCard>
            <IonCard className="event-detail-card">
              <IonCardContent style={{ display: "flex", flexDirection: "row", justifyContent: "space-between" }}>
                <div style={{ display: "flex", flexDirection: "row", rowGap: "10px", alignItems: "center" }}>
                  <IonIcon icon={locationOutline} size="large" style={{ marginRight: "10px", marginLeft: "10px" }} />
                  <div>
            ...
            {checkIfJourneyExists() ? (
              <>
                ...
                <IonList>
                  {attendees.length > 0 &&
                    attendees.sort((a, b) => a.id.localeCompare(b.id)).map((attendee) => (
                      <IonItem
                        key={attendee.id}
                        button
                        onClick={() => {
                      ...
                      >
                        <IonLabel>
                          {attendee.name} {attendee.id === "0" && <IonChip color="primary">You</IonChip>}
                        </IonLabel>
                        <IonNote slot="end">
